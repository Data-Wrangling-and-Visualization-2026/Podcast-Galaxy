...
// Компонент для описания с кнопкой "see more"
const DescriptionWithSeeMore: React.FC<{ description: string }> = ({ description }) => {
    ...
    return (
        <div className="tooltip-description-wrapper">
            <p className="tooltip-description">{displayText}</p>
...
const PodcastMap: React.FC<PodcastMapProps> = ({
    ...
    // Найдите строку с useState для viewState и измените zoom с 1 на 3-4
    const [viewState, setViewState] = useState({
        longitude: 0,
        latitude: 0,
        zoom: 3.5,        // было 1, теперь 3.5 - ближе к карте
        pitch: 0,
        bearing: 0
    });
    ...
    const handleClick = useCallback((info: any) => {
        ...
        if (info.object && info.object.episode_id) {
            ...
            const tooltipElement = document.querySelector('.episode-tooltip');
            if (tooltipElement) {
                tooltipElement.classList.add('animate-in');
                setTimeout(() => {
                    tooltipElement.classList.remove('animate-in');
                }, 300);
            }
        } else {
    ...
    }, [loadEpisodeDetails, onPointClick, onEpisodeDetails, createParticles, createRipple, triggerGlowEffect]);
                    ...
                    {isLoadingDetails ? (
                        ...
                        <>
                            ...
                            <div className="tooltip-podcast">
                                <span className="podcast-label">Podcast title</span>
